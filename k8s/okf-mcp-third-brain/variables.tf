variable "namespace" {
  type    = string
  default = "okf-mcp-third-brain"
}

variable "image" {
  description = "okf-mcp image (same image as the second-brain instance — notebook is env-selected)"
  type        = string
...
variable "hostname" {
  ...
  type        = string
  default     = "notes-mcp-tb.int.gaelrozario.com"
}
